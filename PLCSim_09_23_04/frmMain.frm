...
Private Sub LoadVariables()
  Dim i As Integer
  For i = 0 To 7
    g_uIn(i).Symbol = "IN" & CStr(i)
    g_uOut(i).Symbol = "OUT" & CStr(i)
  Next i
End Sub
...
Private Sub tmrUpdate_Timer()
  ...
  For i = 0 To g_nLines
    ...
    If i = 1 Then frmMain.Caption = g_sCode(i).Substitute
  Next i
  ...
  For i = 0 To g_nLines - 1
    ...
    Else
      ...
      If nEqual > 0 Then
        ...
        For j = 0 To 7
          If g_sCode(i).ResultString = g_uOut(j).Symbol Then
            g_uOut(j).Value = nRet
          End If
        Next j
      End If
      ...
      If InStr(1, sTokens(i), "IN") > 0 Then
        ...
        If Left(sTokens(i), 1) = "!" Then 'must process not symbol before absolute
          For j = 0 To 7
            ...
            If sSymbol = g_uIn(j).Symbol Then
...
'determines output based upon solution
...
Private Sub ReadInputs()
  ...
  For i = 0 To 7
    If chkIn(i).Value = vbChecked Then
      'g_byIn = g_byIn Or (2 ^ i)
      g_uIn(i).Value = chkIn(i).Value
      'If g_byIn Or (2 ^ i) > 0 Then
      '  g_uIn(i).Value = 1
      'Else
      '  g_uIn(i).Value = 0
      'End If
...
Private Function CleanUpValidateCode(sIn As String) As String
  ...
  'must be valid input or output..replaces all operators with '*'
  For i = 1 To Len(sIn)
