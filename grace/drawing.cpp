...
extern vector <ASTEROID> ast;
GLUquadricObj *g_disk = NULL;
...
void LoadAsteroids(void)
{
  ...
  for(int i=0;i<100;i++)
  {
    dist = 1.6 + (float)(rand() % 2);    
    angle = (float)(rand() % 360);
    angle *= 3.14/180;
    ast[i].x = sin(angle) * dist;
    ast[i].y = (float)rand()/32768 *.01;
    ast[i].z = -7 + -cos(angle) * dist;
    ast[i].radius = (float)rand()/32768 *.03;
  }
}
  ...
  glPushMatrix();
    for(int i=0;i<100;i++)
    {
      glPushMatrix();
        ...
        glutSolidSphere(ast[i].radius,10,5);  
      glPopMatrix();
    }
  glPopMatrix();
