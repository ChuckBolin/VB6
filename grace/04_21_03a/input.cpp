	  ...
	  switch (input.key) {
		...
		/*  case GLUT_KEY_LEFT : gFPerson.hdg -= 1;break;
		  ...
		  case GLUT_KEY_DOWN : gFPerson.vel -= .02;break;*/
 		  case GLUT_KEY_LEFT :
        gFPerson.hdg_rate -= .02;
        if (gFPerson.hdg_rate < -gFPerson.hdg_rate_max)
          gFPerson.hdg_rate += .02;
        break;
		  case GLUT_KEY_RIGHT :
        gFPerson.hdg_rate += .02;
        ...
        break;
		  case GLUT_KEY_UP :
        gFPerson.vel += .002;
        if (gFPerson.vel > gFPerson.vel_max)
          gFPerson.vel -= .002;
        break;
		  case GLUT_KEY_DOWN :
        gFPerson.vel -= .002;
        if (gFPerson.vel < gFPerson.vel_min)
          gFPerson.vel += .002;
        break;
...
void CInput::NormalKeyDown(PROGRAMINPUT input){
  ...
  //toggle grid - G or g
  if ((input.uckey == 71) || (input.uckey == 103))
  {
  ...
  //toggle stars - S or s
  if ((input.uckey == 83) || (input.uckey == 115))
  {
    if (gGame.stars ==true)
      gGame.stars = false;
    else
  ...
  //toggle asteroids- A or a
  if ((input.uckey == 65) || (input.uckey == 97))
  {
  ...
  //toggle planet - P or p
  if ((input.uckey == 80) || (input.uckey == 112))
  {
  ...
  //toggle moons = M or m
  if ((input.uckey == 77) || (input.uckey == 109))
  {
    if (gGame.moons ==true)
      gGame.moons = false;
    else
  ...
  //toggle HUD (heads up display) H or h
  if ((input.uckey == 72) || (input.uckey == 104))
  {
    if (gGame.hud ==true)
      gGame.hud = false;
    else
  ...
  // Pause game with 'P' or 'p' key
  
  if ((input.uckey == 112) || (input.uckey == 80))
  {
     if(gGame.GetGameMode() == GAMEMODE_PAUSE)
       gGame.GetGameMode() == GAMEMODE_PLAY;
     else
     {
       gGame.GetGameMode() == GAMEMODE_PAUSE;  
