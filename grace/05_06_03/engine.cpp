...
GLfloat position [] = { 200.0, 0, 0.0, 0.0 };

//planet light source
//GLfloat specular2 [] = { 1.0, 0, 0, 1.0 };
//GLfloat shininess2 [] = { 122.0 };
//GLfloat position2 [] = { 0,0 , 0.0, 0.0 };
...
extern vector <ASTEROID> ast[100];
*/
//****************************************************************************
//  Initialization
//****************************************************************************
...
void InitializeVariables()
{
  ...
  gGame.grid_max_z = 25;
  gGame.grid_red = 0.4;
  gGame.grid_green = 0.4;
  gGame.grid_blue = 0.4;
  gGame.planet = true;
  gGame.moons = true;
  gGame.asteroids = true;
  gGame.hud = true;
  gGame.star_bmp = 2;
  gGame.icon_bmp = 1;
  gGame.font_bmp = 0;
...
void RenderScene(void) {
  ...
  if(gGame.GetGameMode() == GAMEMODE_PLAY)
  {
    ...
    if (gGame.stars == true)
    {
      ...
      glPushMatrix();
        glEnable(GL_TEXTURE_2D);
        glBindTexture ( GL_TEXTURE_2D, gGraphics.texture_objects[gGame.star_bmp]);
        float a,b,c;
        a = gFPerson.x;
        b = gFPerson.y;
        c = gFPerson.z;
        ...
        for(int i=-1;i < 1;i++)
        {
          ...
          {
          glTexCoord2f(0.1,.9);
          glVertex3f(a - i*200,b + j*200,c -200);
          glTexCoord2f(0.1,0.1); 
          glVertex3f(a - i*200,b + (j+1) * 200,c -200);
          glTexCoord2f(.9,0.1);
          glVertex3f(a - (i+1)*200,b + (j+1) * 200,c -200);
          glTexCoord2f(.9,.9);
          glVertex3f(a - (i+1)*200,b + j * 200,c -200);
        }      
      }
      //eastern face
      for(int i=-1;i < 1;i++)
      {
        ...
        {
          glTexCoord2f(0.1,.9);
          glVertex3f(a + 200,b + j*200,c  - i*200);
          glTexCoord2f(0.1,0.1); 
          glVertex3f(a + 200 ,b + (j+1) * 200,c - i*200);
          glTexCoord2f(.9,0.1);
          glVertex3f(a + 200,b + (j+1) * 200,c - (i+1)*200);
          glTexCoord2f(.9,.9);
          glVertex3f(a + 200,b + j * 200,c - (i+1)*200);
        }      
      }
  ...
  if (gGame.grid_on == true)
  {
    glEnable(GL_LIGHTING);
    glBegin(GL_LINES);
      glColor3f(gGame.grid_red,gGame.grid_green,gGame.grid_blue);
      for (int i = gGame.grid_min_x; i < gGame.grid_max_x;i++)
      {
        glVertex3f(i,-1,gGame.grid_min_z);
        glVertex3f(i,-1,gGame.grid_max_z);
      }
      for (int j = gGame.grid_min_z;j < gGame.grid_max_z;j++)
      {
        glVertex3f(gGame.grid_min_x,-1,j);   
        glVertex3f (gGame.grid_max_x,-1,j);   
      }
    glEnd();
    glDisable(GL_LIGHTING);
  } 
    ...
    {    
      ...
      glPushMatrix();
        glLoadIdentity();
        glColor3f(0,0,0);
        DrawHUD(math.RadToDeg(gFPerson.heading),gFPerson.hdg, math.RadToDeg(gFPerson.elevation));
      glPopMatrix();
      glDisable(GL_LIGHTING);
    }
    ...
    glPushMatrix();
      ...
      //gGraphics.DrawText2D(10,518,20,"Elevation: " + FloatToString(math.RadToDeg(gFPerson.elevation)));
...
void ProcessScriptFile(void)
{
  ...
  for(int i=0;i< gScript.GetSize();i++)
  {
    ...
    if (par.name == "fperson_vel_max")
      gFPerson.vel_max = math.StringToFloat(par.argv[0]);
    if (par.name == "fperson_vel_min")
      gFPerson.vel_min = math.StringToFloat(par.argv[0]);
    ...
    if (par.name == "fperson_hdg_rate_max")
      gFPerson.hdg_rate_max = math.StringToFloat(par.argv[0]);
    ...
    {
      gGame.grid_min_x = math.StringToInteger(par.argv[0]);
    }
    if (par.name == "grid_max_x")
    {
      gGame.grid_max_x = math.StringToInteger(par.argv[0]);
    }
    if (par.name == "grid_min_z")
    {
      gGame.grid_min_z = math.StringToInteger(par.argv[0]);
    }
    if (par.name == "grid_max_z")
    {
      gGame.grid_max_z = math.StringToInteger(par.argv[0]);
    }
    if (par.name == "grid_red")
    {
      gGame.grid_red = math.StringToFloat(par.argv[0]);
    }
    if (par.name == "grid_green")
    {
      gGame.grid_green = math.StringToFloat(par.argv[0]);
    }
    if (par.name == "grid_blue")
    {
      gGame.grid_blue = math.StringToFloat(par.argv[0]);
    }
    ...
    if (par.name == "bmp_icon")
    {
      //gGame.icon_bmp = par.argv[0];    
    }

    if (par.name == "bmp_stars")
    {
      //gGame.star_bmp = par.argv[0];    
    }
...
int main(int argc, char **argv)
{
  ...
  gEventLog.LogData("App Path: " + gGame.app_path,0);
  
  //load font info
  int ret = gGraphics.CreateFont(gGame.font_bmp);
  switch(ret)
  {
    case 0:
      gEventLog.LogData("Font bitmap number too low!", 0);
      break;
    case 1:
      gEventLog.LogData("Font bitmap number too high!", 0);
      break;
    case 2:
      gGraphics.LoadFont();
      gEventLog.LogData("Font bitmap loaded",0);
      break;
    default:
      gEventLog.LogData("Unknown font bmp error",0);
      exit(0);
