...
Begin VB.Form frmMain 
   ...
   Caption         =   "Patterns v0.5 (Modified Tetris Clone) - Written by Chuck Bolin, November 2003"
   ClientHeight    =   8160
   ClientLeft      =   150
   ClientTop       =   720
   ClientWidth     =   9840
...
'v0.5 - Supports Mouse, Changed some patterns, fixed bug when playing without starting, gives credit
'         for consecutive rows created
...
Private mblnKey As Boolean ' if true then keyboard controls patterns...otherwise its the mouse
Private mblnStart As Boolean
Private intGameTime As Integer

Private Sub cmdDestroy_Click()

 'piece disappears
  If mblnPaused = False And mblnKey = False And mblnStart = True Then
    If glngScore > 0 Then
...
Private Sub Command1_Click()
  ...
  If Timer1.Enabled = True Then 'stop button
     ...
     optMouse.Enabled = True
     optKeyboard.Enabled = True
     SaveScores
     mblnStart = False
  Else                                      'start button
    ...
    optMouse.Enabled = False
    optKeyboard.Enabled = False
    mblnStart = True
    Timer3.Enabled = True
    intGameTime = 0
    UpdateGameTime
...
Private Sub LoadValues()
  ...
  p(4).N = 2 ^ 24 + 2 ^ 19 + 2 ^ 18 + 2 ^ 17 'L shape
  p(5).N = 2 ^ 22 + 2 ^ 19 + 2 ^ 18 + 2 ^ 17 'L shape
  ...
  p(20).N = 2 ^ 23 + 2 ^ 18 + 2 ^ 12 '1 on 2
  p(21).N = 2 ^ 23 + 2 ^ 18 + 2 ^ 14 '1 on 2 mirrow
  p(22).N = 2 ^ 24 + 2 ^ 19 + 2 ^ 18 + 2 ^ 17 + 2 ^ 12 'S shape mirrow
  p(22).N = 2 ^ 22 + 2 ^ 19 + 2 ^ 18 + 2 ^ 17 + 2 ^ 14 'S shape
  p(23).N = 2 ^ 23 + 2 ^ 19 + 2 ^ 17 ' 1 on 2..inverted V
  p(24).N = 2 ^ 24 + 2 ^ 23 + 2 ^ 19 + 2 ^ 17 '2 on 2..with 1 space in middle of bottom
  p(25).N = 2 ^ 23 + 2 ^ 22 + 2 ^ 19 + 2 ^ 17 'save as above..mirrow
  p(26).N = 2 ^ 24 + 2 ^ 20   '2 pieces with 3 empty in the middle
  p(27).N = 2 ^ 24 + 2 ^ 19 + 2 ^ 14 + 2 ^ 13 + 2 ^ 12 'angle iron
  p(28).N = 2 ^ 24 + 2 ^ 23 + 2 ^ 22 + 2 ^ 21 + 2 ^ 20 + 2 ^ 17 'wide T
  p(29).N = 2 ^ 23 + 2 ^ 21 + 2 ^ 19 + 2 ^ 18 + 2 ^ 17 + 2 ^ 16 + 2 ^ 15 '2 on 5..space in middle top
  p(30).N = 2 ^ 22 + 2 ^ 18 + 2 ^ 17 + 2 ^ 16 + 2 ^ 14 + 2 ^ 13 + 2 ^ 12 + 2 ^ 11 + 2 ^ 10 '1 on 3 on 5
  ...
  mblnKey = True 'keyboard is default controlling device
...
Private Sub pic_KeyDown(KeyCode As Integer, Shift As Integer)
   ...
   If porient = 1 Or porient = 3 Then '0 degrees or 180 degrees
  ...
  If KeyCode = vbKeyUp And mblnPaused = False And mblnKey = True And mblnStart = True Then
  ...
  'piece disappears
  If KeyCode = vbKeySpace And mblnPaused = False And mblnKey = True And mblnStart = True Then
    If glngScore > 0 Then
      ErasePattern2 CInt(prow), CInt(pcol), CInt(ppattern), CInt(porient)
      prow = 1
      pcol = (gintLevel * 2) + Rnd * (16 - (gintLevel * 3))
  ...
  'control key..changes dropping speed to 1x and 3x
  If Shift = 2 And mblnKey = True And mblnStart = True Then
    mblnFast = Not mblnFast
    If mblnFast = True Then
      Timer1.Interval = 200 - gintLevel * 15
      lblTime.Caption = "Speed:    3 X"
    ...
    Else
      Timer1.Interval = 600 - gintLevel * 15
      lblTime.Caption = "Speed:    1 X"
      lblTime.ForeColor = vbGreen
    End If
  End If
  
  'pauses game
  If KeyCode = vbKeyP And mblnKey = True And mblnStart = True Then
    Timer1.Enabled = Not Timer1.Enabled
    Timer2.Enabled = Not Timer2.Enabled
    mblnPaused = Not mblnPaused
  End If
  ...
  'drops part
  If KeyCode = vbKeyDown And mblnPaused = False And mblnKey = True And mblnStart = True Then
    mblnDrop = True
    Do
      Timer1_Timer
  ...
  'move to left
  If KeyCode = vbKeyLeft And mblnPaused = False And mblnKey = True And mblnStart = True Then
    If OkayLeft Then
      ErasePattern2 CInt(prow), CInt(pcol), CInt(ppattern), CInt(porient)
      pcol = pcol - 1
  ...
  'move to right
  If KeyCode = vbKeyRight And mblnPaused = False And mblnKey = True And mblnStart = True Then
    If OkayRight Then
      ErasePattern2 CInt(prow), CInt(pcol), CInt(ppattern), CInt(porient)
      pcol = pcol + 1
...
Private Sub pic_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)
  ...
  'right button changes orientation...rotates counter clockwise
  If mblnPaused = False And mblnKey = False And Button = 2 And mblnStart = True Then
  ...
  'left button drops part
  If mblnPaused = False And mblnKey = False And Button = 1 And mblnStart = True Then
    mblnDrop = True
    Do
      Timer1_Timer
    Loop Until mblnDrop = False
...
Private Sub pic_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
  ...
  col = CInt(X * 20 / pic.width) + 1
  If col < 1 Then col = 1
  If col > 20 Then col = 20
   
  If mblnPaused = False And mblnKey = False And mblnStart = True Then
      
      'determine one step movement...left or right...not instantaneous
      ErasePattern2 CInt(prow), CInt(pcol), CInt(ppattern), CInt(porient)
      If col < pcol Then
        pcol = pcol - 1
      ElseIf col = pcol Then
      ElseIf col > pcol Then
        pcol = pcol + 1
        If pcol > 21 - width Then pcol = 21 - width
      End If
...
Private Sub Timer1_Timer()
  ...
  Dim intRows As Integer 'rows score in sequence
  ...
  For X = 20 To 1 Step -1
    If bd(X) = 1048575 Then 'row found
      ...
      intRows = intRows + 1
      blnFound = True
      glngScore = glngScore + CLng(gintLevel * 10 * gintRows * intRows)
      lblRows.Caption = gintRows
      lblScore.Caption = glngScore
...
'updates total game time
Private Sub Timer3_Timer()
  If mblnPaused = False Then
    intGameTime = intGameTime + 1
    UpdateGameTime
  End If
End Sub
