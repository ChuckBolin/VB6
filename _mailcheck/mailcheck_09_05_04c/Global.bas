...
'**************************************** LookForSubjectFinance +2
Public Function LookForSubjectFinance(strIn As String) As Single
  LookForSubjectFinance = 0
  ' remove spaces
  Dim sTemp As String, nFound As Long
  ...
  For i = 1 To UBound(g_sFinance)
    If InStr(1, g_sFinance(i), "*") < 1 Then
      If InStr(1, sTemp, g_sFinance(i)) > 0 Then
        LookForSubjectFinance = LookForSubjectFinance + 2
        'Exit For
      End If
    Else  ' evaluate wild card *
...
'**************************************** LookForSubjectPorn +2
Public Function LookForSubjectPorn(strIn As String) As Single
  LookForSubjectPorn = 0
  ' remove spaces
  Dim sTemp As String, nFound As Long
  ...
  For i = 1 To UBound(g_sPorn)
    If InStr(1, g_sPorn(i), "*") < 1 Then
      If InStr(1, sTemp, g_sPorn(i)) > 0 Then
        LookForSubjectPorn = LookForSubjectPorn + 2
        'Exit For
      End If
    Else  ' evaluate wild card *
...
'**************************************** LookForSubjectMisc +2
Public Function LookForSubjectMisc(strIn As String) As Single
  LookForSubjectMisc = 0
  ' remove spaces
  Dim sTemp As String, nFound As Long
  ...
  For i = 1 To UBound(g_sMisc)
    If InStr(1, g_sMisc(i), "*") < 1 Then
      If InStr(1, sTemp, g_sMisc(i)) > 0 Then
        LookForSubjectMisc = LookForSubjectMisc + 2
        'Exit For
      End If
    Else  ' evaluate wild card *
...
'**************************************** LookForSubjectHWSW +2
Public Function LookForSubjectHWSW(strIn As String) As Single
  LookForSubjectHWSW = 0
  ' remove spaces
  Dim sTemp As String, nFound As Long
  ...
  For i = 1 To UBound(g_sHWSW)
    If InStr(1, g_sHWSW(i), "*") < 1 Then
      If InStr(1, sTemp, g_sHWSW(i)) > 0 Then
        LookForSubjectHWSW = LookForSubjectHWSW + 2
        'Exit For
      End If
    Else  ' evaluate wild card *
...
'**************************************** LookForSubjectDegree +2
Public Function LookForSubjectDegree(strIn As String) As Single
  LookForSubjectDegree = 0
  ' remove spaces
  Dim sTemp As String, nFound As Long
  ...
  For i = 1 To UBound(g_sDegree)
    If InStr(1, g_sDegree(i), "*") < 1 Then
      If InStr(1, sTemp, g_sDegree(i)) > 0 Then
        LookForSubjectDegree = LookForSubjectDegree + 2
        'Exit For
      End If
    Else  ' evaluate wild card *
...
'**************************************** LookForSubjectAttract +2
Public Function LookForSubjectAttract(strIn As String) As Single
  LookForSubjectAttract = 0
  ' remove spaces
  Dim sTemp As String, nFound As Long
  ...
  For i = 1 To UBound(g_sAttract)
    If InStr(1, g_sAttract(i), "*") < 1 Then
      If InStr(1, sTemp, g_sAttract(i)) > 0 Then
        LookForSubjectAttract = LookForSubjectAttract + 2
        'Exit For
      End If
    Else  ' evaluate wild card *
      If GetPatternInStr(sTemp, g_sFinance(i), "*") > 0 Then
...
'**************************************** LookForSubjectHoliday +2
Public Function LookForSubjectHoliday(strIn As String) As Single
  LookForSubjectHoliday = 0
  ' remove spaces
  Dim sTemp As String, nFound As Long
  ...
  For i = 1 To UBound(g_sHoliday)
    If InStr(1, g_sHoliday(i), "*") < 1 Then
      If InStr(1, sTemp, g_sHoliday(i)) > 0 Then
        LookForSubjectHoliday = LookForSubjectHoliday + 2
        'Exit For
      End If
    Else  ' evaluate wild card *
...
Public Function LookForReceivedUnknown(sIn As String) As Single
  ...
  If InStr(1, sTemp, "fromunknown") Then LookForReceivedUnknown = 2
  If InStr(1, sTemp, "fromunnobody") Then LookForReceivedUnknown = 2
End Function
...
Public Function LookForMissingDate(sIn As String)
  ...
  If Len(sIn) < 1 Then LookForMissingDate = 2
End Function
...
Public Function GetPatternInStr(strIn As String, sPattern As String, sWildCard As String) As Long
  ...
  Dim i As Integer, j As Integer, nLet As Integer, nCount As Integer
