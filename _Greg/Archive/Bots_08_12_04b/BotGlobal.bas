...
Public Const MODE_GO_STRAIGHT = 0
Public Const MODE_AVOID_GO_LEFT = 1
Public Const MODE_AVOID_GO_RIGHT = 2
...
Public Sub InitializeBots()
  ...
  For i = 0 To MAX_BOTS - 1
    ...
    bot(i).ActionMode = MODE_GO_STRAIGHT
...
Public Sub UpdateBots()
  ...
  Dim nX As Single 'obstacle closest X
  Dim nY As Single
  
  For i = MAX_BOTS To MAX_BOTS
    If bot(i).Found = False Then
      ...
      For j = 0 To MAX_BOTS
        ...
        If i <> j Then  'evaluate other bots
          If ContactWithinRange(bot(i).X, bot(i).Y, bot(j).X, bot(j).Y, 8) = True Then
            If ContactWithinAngularView(bot(i).X, bot(i).Y, bot(i).Direction, bot(j).X, bot(j).Y, PI / 4) = True Then
              If GetTargetDirection2D(bot(i).X, bot(i).Y, bot(j).X, bot(j).Y) < GetTargetDirection2D(bot(i).X, bot(i).Y, bot(i).TX, bot(i).TY) Then
                bot(i).Obstacle = True 'something in the way
                nObstacleCt = nObstacleCt + 1
                If bot(j).X < nX Then nX = bot(j).X
                If bot(j).Y < nY Then nY = bot(j).Y
                If GetTargetDirection2D(bot(i).X, bot(i).Y, nX, nY) > GetTargetDirection2D(bot(i).X, bot(i).Y, bot(i).TX, bot(i).TY) Then
                  bot(i).cx = GetStarboard_CX_CY(bot(i).X, bot(i).Y, bot(i).Diameter, nX, nY, bot(j).Diameter).X
                  bot(i).cy = GetStarboard_CX_CY(bot(i).X, bot(i).Y, bot(i).Diameter, nX, nY, bot(j).Diameter).Y
                  bot(i).ActionMode = MODE_AVOID_GO_LEFT
                Else
                  bot(i).cx = GetPort_CX_CY(bot(i).X, bot(i).Y, bot(i).Diameter, nX, nY, bot(j).Diameter).X
                  bot(i).cy = GetPort_CX_CY(bot(i).X, bot(i).Y, bot(i).Diameter, nX, nY, bot(j).Diameter).Y
                  bot(i).ActionMode = MODE_AVOID_GO_RIGHT
   ...
   End If
    ...
    If nObstacleCt > 0 Then
      bot(i).Obstacle = True
      bot(i).Safe = False
    End If
      
      'calc direction differently...collision or not
      If bot(i).Safe = True Then
        bot(i).Direction = GetTargetDirection2D(bot(i).X, bot(i).Y, bot(i).TX, bot(i).TY)
        bot(i).ActionMode = MODE_GO_STRAIGHT
      ElseIf bot(i).Safe = False Then
   ...
   End If
      ...
      Else
        If GetTargetDistance2D(bot(i).X, bot(i).Y, bot(i).cx, bot(i).cy) < 2 Then bot(i).Safe = True
      End If
