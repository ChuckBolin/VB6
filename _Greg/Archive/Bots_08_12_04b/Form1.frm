...
Private Sub DrawBots()
  ...
  For i = 0 To MAX_BOTS
    ...
    If chkBots.Value = vbChecked Then
      ...
      shpBot(i).FillColor = QBColor(bot(i).BotType + 9)
      
      'change color
      If i = MAX_BOTS Then
        If bot(i).ActionMode = MODE_GO_STRAIGHT Then
          shpBot(i).FillColor = vbYellow
        ElseIf bot(i).ActionMode = MODE_AVOID_GO_LEFT Then
          shpBot(i).FillColor = vbRed
        ElseIf bot(i).ActionMode = MODE_AVOID_GO_RIGHT Then
          shpBot(i).FillColor = vbGreen
        End If
      End If
    Else
...
Private Sub vsbBot_Change()
  ...
  lblCollision.Caption = bot(vsbBot.Value).Obstacle
End Sub
