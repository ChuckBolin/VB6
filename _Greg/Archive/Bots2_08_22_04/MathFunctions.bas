...
'Returns coordinate pair that will
'be point around an obstacle.
Public Function GetStarboard_CX_CY(s As Single, t As Single, r1 As Single, u As Single, v As Single, r2 As Single) As COORDINATE_PAIR
  Dim nAngle As Single
  Dim nDir As Single
  Dim cx As Single
  Dim cy As Single
  ...
  nDir = GetTargetDirection2D(s, t, u, v)
  nAngle = nAngle - nDir
  If nAngle < 0 Then nAngle = nAngle + 2 * PI
  nDistance = Sqr((r1 + r2) ^ 2 + (GetTargetDistance2D(s, t, u, v)) ^ 2)
  cx = s + nDistance * Cos(nAngle)
  cy = t + nDistance * Sin(nAngle)
  GetStarboard_CX_CY.X = cx
  GetStarboard_CX_CY.Y = cy
