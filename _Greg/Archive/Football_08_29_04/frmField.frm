...
Private Sub tmrUpdate_Timer()
  ...
  For i = 1 To MAX_BOTS
    If b.AtTarget(i) = True Then
      bRet = b.SetVelocity(i, 0)
    End If
  Next i
  
  'position players
  For i = 1 To 12
    shpHome(i - 1).Left = b.GetX(i)
    shpHome(i - 1).Top = b.GetY(i)
    shpHome(i - 1).Width = 0.75
    shpHome(i - 1).Height = 0.75
    shpHome(i - 1).BorderColor = vbRed
    
    shpVisitor(i - 1).Left = b.GetX(i + 12)
    shpVisitor(i - 1).Top = b.GetY(i + 12)
    shpVisitor(i - 1).Width = 0.75
    shpVisitor(i - 1).Height = 0.75
    shpVisitor(i - 1).BorderColor = vbBlue
    'MsgBox i & " " & b.GetY(i + 12)
  Next i
