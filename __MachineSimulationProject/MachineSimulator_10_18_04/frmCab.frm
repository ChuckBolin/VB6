...
Private Sub picMeter_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)
  frmMain.Caption = X & " " & Y
  If Sqr((X - 960) ^ 2 + (1590 - Y) ^ 2) < 100 Then
    m_nMeterMode = OFF
    lblMode.Caption = ""
    shpDot.Left = 840
    shpDot.Top = 1470
  ...
  End If
  If Sqr((X - 1350) ^ 2 + (1395 - Y) ^ 2) < 100 Then
    m_nMeterMode = VAC
    lblMode.Caption = "VAC"
    shpDot.Left = 1230
    shpDot.Top = 1275
  ...
  End If
  If Sqr((X - 1725) ^ 2 + (1590 - Y) ^ 2) < 100 Then
    m_nMeterMode = VDC
    lblMode.Caption = "VDC"
    shpDot.Left = 1605
    shpDot.Top = 1470
...
Private Sub picTrans_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)
  ...
  frmMain.Caption = X & "  " & Y
  If m_bMoveMeter = False Then
    If HotspotBig(X, Y, 570, 660) Then m_nVolt = g_nHotV * -CInt(v(V_T1_H1))
    If HotspotBig(X, Y, 1365, 660) Then m_nVolt = g_nHotV * -CInt(v(V_T1_H2))
    If HotspotBig(X, Y, 2775, 660) Then m_nVolt = g_nHotV * -CInt(v(V_T1_H3))
    If HotspotBig(X, Y, 3525, 660) Then m_nVolt = g_nHotV * -CInt(v(V_T1_H4))
    If HotspotBig(X, Y, 570, 3450) Then m_nVolt = g_nHotV * -CInt(v(V_T1_X1))
    If HotspotBig(X, Y, 3525, 3450) Then m_nVolt = g_nHotV * -CInt(v(V_T1_X4))
...
Private Function HotspotBig(x1 As Single, y1 As Single, x2 As Single, y2 As Single) As Boolean
  Hotspot = False
  If Sqr((x1 - x2) ^ 2 + (y1 - y2) ^ 2) < 240 Then Hotspot = True
End Function
