...
Private Sub pic_KeyDown(KeyCode As Integer, Shift As Integer)
  ...
  ElseIf KeyCode = 82 Then 'R key, reset
    bot.X = 10000: bot.Y = 10000: bot.Turn = 0: bot.Velocity = 0: bot.Direction = 1.57
    bot.Energy = 100000
    dr.X = bot.X
    dr.Y = bot.Y
...
Private Sub tmrUpdate_Timer()
  ...
  dr.X = dr.X + dr.VX
  dr.Y = dr.Y + dr.VY
  dr.Velocity = Sqr((dr.VX ^ 2) + (dr.VY ^ 2))
  dr.Direction = GetAngleRadiansArctan(dr.VY, dr.VX)
  
  'draws Dead Reckoning on screen
  DX = dr.X - bot.X
  DY = dr.Y - bot.Y
  'pic.Circle (4000 - DX, 4000 - DY), 128, 0
  ...
  ElseIf bot.Velocity < 0 Then
    pic.ForeColor = vbRed
  End If
  pic.Line (4000, 4000)-(4000 + (25 * bot.VX), 4000 + (25 * bot.VY))
