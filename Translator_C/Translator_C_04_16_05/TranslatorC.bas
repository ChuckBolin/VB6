...
' C_For_Oneliner(sInput) converts 'for' to VM without braces
    ...
    If InStr(1, sLines(i), "for") > 0 Then 'for found
      ...
      If bReturn = True Then
        ...
        Else  'two liner - lets parse
          nPos1 = InStr(1, sLines(i), "(")
          nPos2 = InStr(nPos1 + 1, sLines(i), ";")
          If nPos2 > nPos1 And nPos1 > 0 Then  '1st 'for' parameter
            nEqual = InStr(nPos1, sLines(i), "=")
            If nEqual > nPos1 And nEqual < nPos2 Then
              sVar = Trim(Mid(sLines(i), nPos1 + 1, nEqual - nPos1 - 1))
              sValue = Trim(Mid(sLines(i), nEqual + 1, nPos2 - nEqual - 1))
              If Len(sVar) < 1 Or Len(sValue) < 1 Then
                C_For_Twoliner = "ERROR!" & vbCrLf & "   Invalid parameter for 'for'!"
                Exit Function
              End If
              sOut = sOut & "SVAR " & sVar & "," & sValue & vbCrLf
              nPos1 = nPos2
              nPos2 = InStr(nPos1 + 1, sLines(i), ";")
              If nPos2 > nPos1 Then            '2nd 'for' parameter
                sExpr = Mid(sLines(i), nPos1 + 1, nPos2 - nPos1 - 1)
                sOut = sOut & "GLR " & sExpr & ",[3]" & vbCrLf
                nPos1 = nPos2
                nPos2 = InStr(nPos1, sLines(i), ")")
                If nPos2 > nPos1 Then
                  sExpr2 = Trim(Mid(sLines(i), nPos1 + 1, nPos2 - nPos1 - 1))
                  sOut = sOut & sExpr2 & ";" & vbCrLf
                End If
              End If
            Else  'no equal sign
