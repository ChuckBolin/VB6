...
Public Function C_DecrementVariable(sIn) As String
  ...
  If nPlus > 1 Then  '-- found
    ...
    sReturn = GetVariableValue(sName)
    ...
    For j = 0 To 12
      ...
      If nPos > 0 Then  'variable type found
        ...
        Else 'variable created, not intialized
          ...
          sReturn = AddVariable(sName, sType, sValue)
...
'******************************************
Public Function AddVariable(sName As String, sType As String, sValue As String)
  Dim i As Integer
...
Public Function IsAutoFunction(sInput As String) As String
  ...
  sInput = Mid(sInput, Len(sCode1) + 1)
  sInput = Trim(sInput)
  sInput = Mid(sInput, 6)
...
Public Function RemoveBlankLines(sInput As String) As String
  ...
  For i = 0 To UBound(sLines) - 1
    ...
    'MsgBox "|" & sLines(i)
    If Len(sLines(i)) > 0 Then   'there is something here...keep this
      sOut = sOut & sLines(i) & vbCrLf
    End If
  Next i
