...
Public Sub Initialize()
  ...
  'load AIP variables
  AI.FullName = "Artificial Intelligence Program (AIP)"
  AI.Version = "0.3b"
  AI.Date = "July 20, 2002"

  'let's grab default global information
  LoadDefaultData
  'AI.Filepath = App.Path & "\"
  'AI.Filename = "game1.aip"
  bFound = LoadGameData(AI.Filepath & AI.Filename)
...
Public Sub LoadDefaultData()
  ...
  'loads game piece data
  Program.Symbol = "X"
  Program.Color = RGB(0, 0, 0)
  Program.ColorInverse = RGB(255, 255, 255)
  Teacher.Symbol = "O"
  Teacher.Color = RGB(0, 0, 0)
  Teacher.ColorInverse = RGB(255, 255, 255)
...
Public Function LoadGameData(sFile As String) As Boolean
  ...
  Open sFile For Input As nFile
    ...
    Do
      ...
      'loads AI program variables
      If Left(sInput, 8) = "PROGRAM." Then
        nEqual = InStr(1, sInput, "=")
        If nEqual > 0 Then
          sProp = Mid(sInput, 9, nEqual - 9) 'grab rest of info beyond
          sVar = Mid(sInput, nEqual + 1) 'actual variable data
          Select Case sProp
            Case "SYMBOL"
              Program.Symbol = sVar
            Case "COLOR"
              Program.Color = CLng(sVar)
            Case "COLOR_INVERSE"
              Program.ColorInverse = CLng(sVar)
          End Select
        End If
      End If

      'loads teacher variables
      If Left(sInput, 8) = "TEACHER." Then
        nEqual = InStr(1, sInput, "=")
        If nEqual > 0 Then
          sProp = Mid(sInput, 9, nEqual - 9) 'grab rest of info beyond
          sVar = Mid(sInput, nEqual + 1) 'actual variable data
          Select Case sProp
            Case "SYMBOL"
              Teacher.Symbol = sVar
            Case "COLOR"
              Teacher.Color = CLng(sVar)
            Case "COLOR_INVERSE"
              Teacher.ColorInverse = CLng(sVar)
          End Select
        End If
      End If
...
Public Function SaveGameData(sFile As String) As Boolean
  ...
  Open sFile For Output As nFile
    ...
    End If
    Print #nFile, "GAME.PATTERN_RANDOM_VALUE=" & CStr(Game.PatternRandomValue) & vbCrLf
    Print #nFile, "'program variables"
    Print #nFile, "PROGRAM.SYMBOL=" & Program.Symbol
    Print #nFile, "PROGRAM.COLOR=" & CStr(Program.Color)
    Print #nFile, "PROGRAM.COLOR_INVERSE=" & CStr(Program.ColorInverse) & vbCrLf
    Print #nFile, "'teacher variables"
    Print #nFile, "TEACHER.SYMBOL=" & Teacher.Symbol
    Print #nFile, "TEACHER.COLOR=" & CLng(Teacher.Color)
    Print #nFile, "TEACHER.COLOR_INVERSE=" & CStr(Teacher.ColorInverse)
