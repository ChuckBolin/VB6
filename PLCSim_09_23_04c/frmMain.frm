...
Private Sub LoadProgram()
  ...
  'place boolean code into text box
  txtCode.Text = ""
...
Private Sub DisplayILCode()
  Dim i As Integer, nEqual As Integer, j As Integer
  Dim sInput As String, sOutput As String, sCode As String
  ...
  For i = 0 To g_nLines - 1
    ...
    If nEqual > 0 Then 'process only if equal sign
      ...
      txtIL.Text = txtIL.Text & ";Rung: " & CStr(i) & vbCrLf
      sTokens = Split(sInput, " ")
      For j = 0 To UBound(sTokens)
        txtIL.Text = txtIL.Text & sTokens(j) & vbCrLf
      Next j
      
      txtIL.Text = txtIL.Text & vbCrLf
    End If
  Next i
...
Private Sub tmrUpdate_Timer()
  ...
  For j = 0 To MAX_INPUTS - 1
    g_uIn(j).Value = 0
  Next j
  ReadInputs  'read into g_uIn(MAX_INPUTS)
...
Private Sub txtCode_Click()
  'If m_nPLCMode = Run Then m_sTextCode = txtCode.Text
End Sub
